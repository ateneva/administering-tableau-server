...
# define the table
hyper_table = TableDefinition(table_name=TableName("Extract", "Extract"))
...
def define_hyper_schema(hyper_bucket_name, hyper_schema):
    ...
    with open(hyper_schema_full_path, "r") as jf:
        ...
        for line in data:
            ...
            field_type = sql_type[
                line["type"]
            ]  # sql_type must be recognized as class object and not STRING
...
def create_hyper_file_from_csv(file_name, download_bucket_name, hyper_file_name):
    ...
    with HyperProcess(
        ...
        with Connection(
            ...
            print(
                f'Files to process: {len([blob for blob in blobs if blob.name.endswith(".csv")])}'
            )
            total_rows = 0
            ...
            for blob in blobs:
                if blob.name.endswith(
                    ".csv"
                ):  # make sure only csv files are dowwloaded
                    ...
                    print(
                        f"The number of rows in table {hyper_table.table_name} is {total_rows}."
                    )
...
def publish_to_server(
    tableau_server_link, tableau_user, user_password, project_name, hyper_extract
):
    ...
    with server.auth.sign_in(tableau_auth):
        ...
        datasource = server.datasources.publish(
            datasource, path_to_hyper_file, publish_mode
        )
        print(
            f"Publishing of datasource '{hyper_extract}' complete. Datasource ID: {datasource.id}"
        )
...
if __name__ == "__main__":
    ...
    define_hyper_schema(
        hyper_schema_details["hyper_bucket_name"], hyper_schema_details["hyper_schema"]
    )
    ...
    try:
        create_hyper_file_from_csv(
            hyper_file_payload["file_name"],
            hyper_file_payload["download_bucket_name"],
    ...
    publish_to_server(
        publish_details["tableau_server_link"],
        publish_details["tableau_user"],
        publish_details["user_password"],
        publish_details["project_name"],
